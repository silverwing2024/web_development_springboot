...
/*
    New project 생성시 주의점 :
        ...
        4. build.gradle 설정을 복사하는데, 복사 후에 ->  sync를 꼭 눌러주셔야 합니다.
            -> SpringBootDeveloperApplication에서 @SpringBootApplication 애너테이션 빨간줄이 있다면 새로 고침 안 했을 확율이 90%
        5. resources 내에 static 내에 index.html 이라고 하는데, 해당 폴더명의 경우 대부분의 개발자들이 합의한 형태(홈페이지가 정적)
        6. 앞으로의 수업 시간에는 new project 생성시에 이렇게 자세히 풀이할 일은 자주 있지 않을 예정이니 꼭 익혀두시기 바랍니다.
            -> github 본다고 되는 부분이 아니니만큼 신경써 주시면 감사하겠습니다.
*/
...
/*
    1. Spring vs. SpringBoot
        ...
        SpringBoot의 등장
            ...
            특징:
                1) 톰캣, 제티, 언더토우 같은 웹 애플리케이션 서버가 내장되 있어 따로 설치하지 않아도 독립적으로 실행 가능
     ...
     2.스프링 컨셉
        ...
        2) 의존성 주입(DI)
            ...
            @Autowired 애너테이션이 중요합니다. -> 스프링 컨테이너에 있는 (Bean)을 주입하는 역할
            Bean : 스프링 컨테이너가 관리하는 객체 -> 추후 한번 더 수업할 예정
        ...
        3) 빈과 스프링 컨테이너
            ...
            3)-2. 빈: 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                스프링은 빈을 스프링 컨테이너에 등록하기 위해 XML 파일 설정, 애너테이션 추가 등의 방법을 제공하는데, 이것이 의미하는 바는
        ...
        4) 관점 지향 프로그래밍(AOP)
            Aspect-Oriented Programming : 프로그래밍에 대한 관심을 핵심 관점 / 부가 관점으로 나누어 모듈화함을 의미

            ex) 계좌 이체, 고객 관리하는 프로그램이 있다고 가정할 때, 각 프로그램에는
            로깅 조직, 즉 지금까지 일어난 일을 기록하기 위한 로직과 여러 데이터를 관리하기 위한 로직과 데이터를 관리
...
3. 스프링 부트 3 둘러보기
    첫번 째 스프링 부트 3 예제 만들기
        01 단계 -
            SpringBootDeveloper 폴더에
            TestController.java 파일을 생성
    ...
    스프링 부트 스타터 살펴보기
        ...
        자주사용하는 스타터의 예시
        spring-boot-starter-web : Spring MVC를 사용해 RESTFul 웹 서비스를 개발할 때 필요한 의존성
        spring-boot-starter-test : 스프링 애플리케이션을 테스트하기 위해 필요한 의존성 모음
        spring-boot-starter-validation : 유효성 검사를 위해 필요한 의존성 모음
        spring-boot-starter-actuator : 모니터링을 위해 애플리케이션에서 제공하는 다양한 정보를 제공하기 윕게 하는 의존성 모음
        spring-boot-starter-jpa: ORM(추후 수업 예정)을 사용하기 위한 인터페이스의 모음인 JPA를 더 쉽게 사용하기 위한 의존성 모음
