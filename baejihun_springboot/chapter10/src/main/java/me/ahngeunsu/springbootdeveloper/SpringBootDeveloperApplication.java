...
/*
    사전 지식 : OAuth
        ...
        관련 용어 정리
            ...
            인증서버(Authorization Server) : 클라이언트에게 리소스 오너의 정보에 접근할 수 있는
                토큰을 발급하는 역할을 하는 애클리케이션을 의미
            ...
            OAuth를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스서버에
                ...
                그렇다면 리소스오너의 젖ㅇ보를 어떻게 취득할 수 있는지에 대한 방법이 필요.

                리소스 오너 정보를 취득하는 네 가지 방법
                    1) 권한 부여 코드 승인 타입(Authorization Code Grant Type) :
                        OAuth 2.0 가장 대중적인 인증 방법. 클라이언트가 리소스에 접근하는 데 사용.
                    ...
                    4) 클라이언트 자격 증명 승인 타입(Client Credentials Grant) :
                        클라이언트 컨텍스트 외부에서 액세스 토큰을 얻어 특정 리소스에 접근을 요청할때 사용하는 방식
     ...
     1. 권한 요청    |       5. 액세스 토큰으로 데이터에 접근     | 인증 서버에
            ...
            보통은 클라이언트ID, 리다이렉트 URI, 응답 타입등을 파라미터로 보냅니다.
            실제 요청에 쓰이는 요청 URI 예시를통해 주요 파라미터를 소개하자면,
            ...
            이때 파라미터에 인증 코드를 함께 제공합니다.
             ...
             🎈 인증 코드 예시
            GET http://localhost:8080/myapp?code=
    ...
    쿠키 관리 클래스를 구현할겁니다.
        OAuth2 인증플로우를 구현하면서 쿠키를 사용할 일이 생깁니다.
        그때마다 쿠키를 생성하고 삭제하는 로직을 추가하면 불편해서
        유틸리티로 사용할 쿠키 관리 클래스를 미리 구현

        springboordeveloper 패키지 내에 util 패키지를 생성 -> CookieUtil.java를 생성
