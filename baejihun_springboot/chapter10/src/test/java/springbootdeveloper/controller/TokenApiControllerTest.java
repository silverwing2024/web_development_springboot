...
/*
    ...
    When
        - 토큰 추가 API에 요청을 보냅니다. 이 때 요청 타입은 JSON이며,
...
/*
    chapter09
        토큰 기반 인증의 특징
        그 중에서 JWT을 적용하고, 토큰 제공자(Token Provider)를 만들었습니다.

        해당 인증 방법은 대중적으로 사용되는데다가
        JWT가 현재 상황에서는 인기가 있는 편입니다.

        Token : 클라이언트를 구분하는데 사용하는 유일한 값
            서버에서 생성한 뒤에 클라이언트에게 제공한 뒤
            클라이언트는 서버에 요청할 때 마다 요청 내용과 함께 토큰 전송,
            서버가 하는 일은 토큰이 유효한 사용자인지 확인하는 것
            -> 세션 로그인(form login)과의 차이
        ...
        Refresh Token : 액세스 토큰과 별개의 토큰. 액세스 토큰이 만료되었을 때
            새로운 액세스 토큰을 발급받는 용도
            (CreateToken Request으로 Regresh Token을 포함해서 요청했고, 그
             결과값으로 Access Token을 반환받는 구조로 구현했었습니다.)
