    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        토큰 기반 인증:
            *토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            여러 요청을 해당 토큰과 함께 신청함. 이 후 서버는 토큰을 확인하여 유효한 사항인지를
            검증하게 됨.
            ...
            토큰 기반 인증의 특징
                1) 무상태성
                    ...
                    그래서 완전한 무상태(statelee)로 효율적인 검증 가능

                2) 확장성
                    무상태성과 관련하여, 서버를 확장할 때 상태 관리를 신경쓸 필요가 없다.
            ...
            JWT(JSON Web Token) -> json 형태로 주고 받음
               ...
               헤더에는 토큰의 타입과 해싱 알고리즘을 지정하는 정보를포함함.
...

               {
                ...
                2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)이라고 하며, 키-값의 한쌍으로 이루어짐.
                    ...
                    공개 클리엠(public claim) : 공개되어도 상관없는 클레임.
                        ...
                        JWT 예씨
                    {
                        ...
                        "iat": 162237068,
                        "exp": 1622372678,
                        "https://ahngeunsu.com/jwt_claims/is_admin// 공개 클레임
        ...
        2. JWT 서비스 구현
            의존성과 토큰 제공자를 추가하고 리프레스 토큰 도메인 토큰 필터를 구현하면
            -> JWT 서비스를 사용할 준비가 됬다고 볼 수 있습니다.


            build.gradle로 이동해서 의존성 추가합니다.
