...
public class TokenProvider {
    ...
    public String generateToken(User user, Duration expiredAt){
        ...
        Date now = new Date();
        // 빨간 줄이 뜨는 상태는 아직 정의하지 않았기 때문입니다. -> 바로 밑에 정의할 예정
    ...
    private String makeToken(Date expiry, User user){
        ...
        return Jwts.builder()
                ...
                .setIssuer(jwtProperties.getIssuer())// import io.jsonwebtoken.Jwts
...
/*
    1. maketoken
        토큰을 생성하는 메서드 argument로 만료시간, 유저 정보를 받았습니다.
        set 계열 메서드들을 통해 여러 값을 지정했는데
        header에는 typ, 내용에 iss, iat, exp, sub
        claim에는 유저 id 지정
        토큰을 마들 때 properties 파일에 선언해둔 비밀 값과 함께 HS256 방식으로 암호화

    2. validToken
        토큰이 유효한지 검증하는 메서드. 프로퍼티즈 파일에 선언한 비밀값과 함꼐 토큰 복호화
    ...
    3. getAuthentication():
        ...
        getClaims()를 호출, 클레임 정보를 반환 받아 사용자 이메일이 들어있는 토큰 제목 sub와 토큰 기반으로 인증 정보를 생성
        이 때, UserPasswordAuthenticationToken의 첫 argument로 들어가는 User는 프로젝트에서 만든 클래스가 아니라, 스프링 시큐리티에서
        제공하는 객체인 User클래스를 임포트 해와야 하기 떄문에 경로를 강제 설정 해뒀습니다.
