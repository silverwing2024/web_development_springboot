...
/*
    서버 실행 안될 때 일단 한번 시도해보기
    1. 햄버거 - build = rebuild project
    2. intellij 전부 다 끄고 재시작
    3. SpringBootDeveloperApplication.java들어와서
    4. current file인지 확인하고 재시작
...
/*
    학습목표: 로그인 / 회원가입 기능을 구현
        회원 가입 구조는 로그인 구조와 유사합니다.

    /login 요청이 들어올 때,
    -> UserViewController가 해당 요청에 대한 분기 처리를 하고 WebSecurityConfig에 설정한 보안관련
        내용들을 실행
    -> UserDetailsService를 실행하면 요청을 성공할 때
    -> defaultSuccessUrl로 설정한 /articlss로 리다이렉트 하거나 csrf를 disable하거나 하는 등의 작업

    userDetailService에서는 loadUserByUsername() 메서드를 실행하여 이메일로 유저를 찾고 변환.
    ...
    그  로그아웃 로직: SecurityContextLogoutHandler에서 제공하는 logout() 메서드를 실행할 예정.

    1. 스프링 시큐리티(Spring Security)
        1) 인증 vs 인가
            (1) 인증(Authentication) : 사용자의 신원을 입증하는 과정
                예를 들어 사용자가 사이트에 로그인할 때 누구인지 확인하는 괒어을 인증이라고 함.
        ...
        2) 스프링 시큐리티
            : 스프링 기반 애플리케이션의 보안을 담당하는 스프링 하위 프레임워크 보안 관련 옵션을 다수
                제공해줌.
                CSRF, 세션 공격을 방어해주고(-> 정처기 등에 자주 나옴), 요청 헤더도 보안 처리를 새주므로
        ...
        3) 필터 기반으로 동작하는 스프링 시큐리티
            ...
            모두 외워야 하는 거 아님. 다만 로그인 과정이 어떤 흐름으로 이어지는 지를 이해하면 더 잘 활용 가능
            일단은 회원 가입/로그인 관련 부분을 코딩으로 풀어나갈 예정

    2. 회원 도메인 만들기
        스프링 시큐리티를 활용한 인증, 인가 구현 예정.
        회원 정보를 저장할 테이블을 만들고,
        테이블과 연결할 도메인을 만들고,
