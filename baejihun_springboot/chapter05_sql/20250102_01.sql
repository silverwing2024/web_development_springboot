...
-- 이전까지는 테이블 전체거나 혹은 특정 컬림에 관련된 부분들을 조회했지만
-- 현업에서는 데이터의 일부 컬럼을 가져오거나 상위 n의 데이터를 조회하는 것
-- 뿐만 아니라 특정 컬럼의 값이 A인 데이터만 가져오는 등 복잡한 쿼리를 작성할 일이 있습니다. 

-- 회원 정보 테이블에서 users에서 거주 국가(country)가 한국(Korea)인 회원만 추출하는 쿼리문
SELECT * FROM users WHERE country = "korea";

--거주 국가가 한국이 아닌 회원만을 추출하는 쿼리문을 작성하시오.
SELECT * FROM users WHERE country != "korea";
...
SELECT  * 
	FROM users
	WHERE country = "USA" AND id=10; 
	-- id가 PK이기 때문에 country 조건은 의미 없음

-- WHERE 절 에서는 여러 조건을 동시에 적용할 수 있고, 조건의 개수에 
-- 제한이 없음. 논리 연산 AND / OR / BETWEEN 
...
-- 			시간값을 조회할 때는 [컬럼명] between [시작날짜] and [종료날짜]
-- 			마찬가지로 시작날짜와 종료날짜를 포함한 모든 것을 출력해준다. 
-- 예제 : 가입일시가 2010-12-01부터 2011-01-01까지인 회원 정보 출력
-- (between을 사용하지 않고)
...
-- 응용: users에서 거주 국가가 KOREA, USA, OK인 회원 정보를 추출하세요.
SELECT * FROM users 
	WHERE country ="korea" OR 
		  country ="USA" OR 
		  country = "UK";  
...
-- 응용: users에서 거주 국가가 KOREA, USA, OK가 아닌 회원 정보를 추출하세요.
SELECT * FROM users 
...
-- 거주 국가가 S로 끝나는 국가(ex)South Korea) 모두를 출력
SELECT *
	FROM users
	WHERE country LIKE "%S"
	;

-- 거주 국가 명에 S가 들어가기만 하면 모두 출력
SELECT *
	FROM users
	WHERE country LIKE "%S%"
	;
...
-- A칼럼의 값이 B이다 일 때 특히 NULL일 때 '=' 대신 사용
-- 예제 : USERS 에서 created_at 칼럼의 값이 null인 결과 출력
 SELECT *
	FROM users u 
 ...
 -- 연습 문제
 	-- 1. users에서 country가 Mexico인 회원 정보 추출.
 		-- 단 created_at, phone, city, country칼럼만 추출할 것 
SELECT created_at, phone, city, country 
	FROM users u
	WHERE country = "Mexico";

 	-- 2. products에서 id가 20 이하이고 price는 30 이상인 제품 정보 출력
 		-- 단 기존 칼럼 전부 출력하고, 정상 가격에서 얼마나 할인 되었는지를 
 		-- (price - discount_price)
 		-- discount_amount라는 칼럼명으로 추출할 것.
...
; 
 	...
 	-- 3. users에서 country가 Korea, Canadam Belgium도 아닌 회원의 정보를
 		-- 모두 추출할 것.
...
SELECT * 
	FROM users
	ORDER BY id ASC;	-- ASC : Ascending의 의미로 '오름차순'이란 뜻
...
-- 예제 : users에서 username, phonem city, country, id 컬럼을 순서대로 출력하는데, 
	-- 첫 번째 컬럼 기준으로 오름차순 정렬 해보세요.

SELECT username, phone, city, country, id 
	FROM users u 
	ORDER BY 1 ASC
	;
-- 아까와 같이 ORDER BY 1 ASC 로 했지만 정렬이 id가 아닌 username을 기준으로 
-- ASCII Code가 적용되어 정렬방식이 달라졌음을 확인 가능 
...
-- order by 정리
-- 1. 데이터를 가저올 때 지정된 컬럼을 기준으로 정렬함.
-- 2. 형식 : ORDER BY 컬럼명 ASC/DESC
-- 3. 2개 이상의 정렬 기준을 쉼표 기준으로 합쳐서 지정 가능
...
SELECT * FROM play_sql.products p 
	ORDER BY price desc
;
...
SELECT *
	FROM 음식점정보
	WHERE 지역 = "마포구"
	AND 1인부가능여부 = 1 -- 1을 쓸 경우에는 True의미/0은 FALSE 의미
	;	
