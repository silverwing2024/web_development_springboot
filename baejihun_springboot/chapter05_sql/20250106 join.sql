...
SELECT SUBSTR(created_at, 1, 7) AS month, count(DISTINCT id) AS uniqueUserCnt
	FROM users
	GROUP BY SUBSTR(created_at, 1, 7)	-- 원래는 month로 썻을 경우 안됬던 것으로 보임
...
-- SQLD/P에서는 2번 쿼리와 같은 방식으로 출제되지 않습니다. 
...
-- user와 orders를 하나로 결합하여 출력합니다. (단 주문정보가 있는 회원의 정보만 출력)
...
-- 추가 문법이 적용됬음. 
-- 회원 정보와 주문 정보를 하나로 결합하기 위해 users와 orders를 INNER JOIN(추후 설명)으로 
-- 묶고, '후속조건'으로 "주문 정보가 있는 회원의 정보만 출력하기 위해" u.id = o.user_id를 적용함.
...
-- FROM users u로 작성했을 때 이후에는 u만 썻을 경우 users 테이블을 의미하게 됨.
...
-- 왜냐면 users내에 있는 id는 1부터 끝까지 있으니까요.
-- users테이브렝는 회원 id가 id 컬럼에 기록되 있고, orders 테이블에는 회원 id가 user_id로 
...
-- 테이블에서 개별 행을 유일하게 구분 짓습니다. 따라서 키 값은 커럼 내에서 중복되지 않으며 
-- 개별행을 구분해야 하므로 null 값을 가질 수 없습니다. 
...
-- 세 번 주문을 했다면 회원 정보(users)와 주문 정보(orders)를 결합한 결과에는 u.id=7인 
-- 행에 3개 있을 것이다.이 경우 '한번이라도 주문한 회원 수'를 중복 없이 구하려면 회원 id를 
-- 중복 제거한 뒤에 회원 수를 count할 필요가 있다.
...
-- 예를 들어서 PK 값이 A,B,C만 있다면 PK값도 이 값만 가질 수 있고, 또한 중복되지 않는다는
-- 특징을 가지고 있음.
-- FK의 경우에는 참조하고 있는 관계에 따라 참조 대상인 PK값이 여러 번 나타날 수 있습니다. 
...
-- 이상의 예시에서는 users 테이블이 왼쪽에 있으므로, users테이블의 모든 결과값을 가져오는데,
-- orders 테이블에 대응하는 값이 없는 경우는 NULL 값으로 출력됩니다.
...
-- u.id, u.username, o.order_date처럼 컬럼이 속한 테이블 별칭을 . 앞에 명시해 주면 됩니다. 
...
-- users와 orders를 하나로 결합하고, 추가로 orderdetails에 있는 데이터도 출력해 보자.(단, 
-- 주문 정보가 없는 회원의 주문 정보도 모두 출력하고, 다음 컬럼을 출력하자. u.id, u.username, 
-- u.phone, o.user_id, o.id, od.order_id, od.product_id). --> JOIN이 두 번 일어납니다. 

-- FROM 내에서는 JOIN을 중첩해서 횟수 제한없이 사용 가능합니다. 
	-- 첫 번째 JOIN의 ON절 뒤에, 두 번째 JOIN절을 작성하면 됩니다. 

SELECT u.id, u.username, u.phone, o.user_id, o.id, od.order_id, od.product_id
	FROM users u LEFT JOIN orders o ON u.id = o.user_id	-- 여기까지가 첫번 째 JOIN
	LEFT JOIN orderdetails od ON o.id = od.order_id 
	;

-- users와 orders를 하나로 결합하여 출력해보자(단, 회원 정보가 없는 주분 정보도 출력).
SELECT *
	FROM users u RIGHT JOIN orders o ON u.id = o.user_id ;
-- RIGHT JOIN은 기본적으로 LEFT JOIN과 기능은 동일합니다. LEFT JOIN 에서는 왼쪽에 위치한 
-- 테이블의 모든 값을 가져왔습니다. (즉, JOIN 했을 때를 기준으로 u.id는 있지만 o.user_id가 없는 경우도 
-- 출력된 다는 것을 의미합니다.)
-- RIGHT JOIN의 경우는 오른쪽 테이블에 위치한 값을 전부 가지고 온다는 것을 의미합니다. 
-- 즉 여기에서는 예제 쿼리의 결과값은 INNER JOIN과 동일하게 됩니다. 

-- 이는 두 테이블 간의 '포함 관계'에서 비롯됩니다. 
-- user테이블에 id가 없는 회원의 정보는 order 테이블에 존재할 수 없음.
-- 따라서 orders 테이블의 user_id 칼럼은 모두 users 테이블의 id 칼럼에 있는 값에 해당합니다.
-- 결합으로 봤을 때 orders는 users 테이블에 종속되 있으므로 null값이 출력될 수 없습니다. 

-- 이상을 이유로, 
-- 많은 기업에선 RIGHT JOIN 대신에 LEFT JOIN을 사용하도록 권장합니다. 
...
SELECT *
	...
	ORDER BY u.id
	;	-- 모든 가능한 행 조합에서 -> CROSS JOIN을 유추
	
-- cross join - 두 테이블 간의 집합을 조합해 만들 수 있는 모든 경우의 수를 생성하는 방식으로, 
	-- 카테시안 곱(cartesian Product)를 의미함.
-- u.id를 기준으로 정렬했습니다. (얼마나 많은 조합이 나오는지 시각적으로 보여드릴 수 있도록 추가함)

-- u.id와 o.user_id를 연결하는 등의 조건 없이 두 테이블의 모든 행을 합쳐서 만들 수 있는 모든 경우의 수를 생성한 것입니다. 
	
-- 즉, 10행의 테이블과 20행의 테이블을 CROSS JOIN 하면 200행이 됩니다. 
-- 해당 경우 CROSS JOIN은 모든 경우의 수를 구하므로ㅒ ON 조건을 굳이 설정하지 않습니다. 
...
-- 연습 문제
-- 1. users와 staff를 참고하여 회원 중 직원인 사람의 회원 id, 이메일, 거주 도시, 거주 국가, 성, 이름을 한 화면에 출력하시오. 

SELECT u.id, u.postalcode, u.city, u.country, s.last_name, s.first_name 
	FROM users u INNER JOIN staff s ON u.id = s.user_id 
	;
	
-- 2. staff와 orders를 참고하여 직원 아이디가 3번, 5번인 직원의 담당 주문을 출력하세요.(단, 직원 아이디, 직원 성, 주문 아이디, 주문 일자만 출력하세요)
SELECT s.id, s.last_name, o.id, o.order_date 
	FROM staff s LEFT JOIN orders o ON o.staff_id = s.id 
	WHERE s.id IN (3,5)
	ORDER BY s.id
	;	-- feat: 20250106 join
...
-- 할인 가격 기준으로 각각 구하세요. (단, 정상 가격 주문 금액의 총합 기준으로 내림차순으로 정렬 하세요.)
SELECT o.user_id, 
	sum(p.price * quantity), sum(p.discount_price * quantity) 
...
-- 일단 JOIN이 수행된 뒤에 다른 명령어들이 수행됩니다. JOIN 사용시, 결합할 두 테이블 사이에
...
-- 정리 2. JOIN 중첩
-- FROM [테이블명1] a(별칭) [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명2] b(별칭)
-- ON [JOIN 조건1]		-> 선호되는 조건 방식 PK = FK
-- [INNER/LEFT/RIGHT/CROSS JOIN][테이블명3] c(별칭)
-- ON [JOIN 조건2]
	
-- FROM 내에서 JOIN을 여러번 ㅈ ㅜㅇ첩  사용 가능. 앞의 JOIN의 ON 절 뒤에 새로운 JOIN 명령어를 
...
-- 그런데 실무에서는 웬만하면 가독성을 위해서 INNER 명시.
...
-- RIGHT JOIN: 오른쪽 테이블의 모든 데이터 값을 결과에 포함(잘 안 씀)
-- FULL OUTER JOIN: 왼쪽과 오른쪽 테이블의 모든 값을 결과에 포함
		-- LEFT + RIGHT JOIN의 결과값이 중복없이 결합된 형태.
		-- DB에 따라 지원하지 않는 경우도 많음.

-- 정리 5. CROSS JOIN
-- FROM  [테이블명1] a cross join [테이블명 2] b
-- cross join은 두 테이블을 결합했을 때 각 테이블의 행으로 만들 수 있는 모든 조합을 결과값으로 도출하는 연산 -> 카테시안 곱이라고 함. -> 쓸 때 주의
	
-- 주의 사항: FULL OUTER JOIN vs. CROSS JOIN
-- FULL OUTER JOIN의 경우 ON 조건에 부합할 때만 결과값을 도출하는 반면에,
-- CROSS JOIN은 모든 경우의 수를 출력하기 때문에 ON 조건을 며시하는 일이 거의 없다.
