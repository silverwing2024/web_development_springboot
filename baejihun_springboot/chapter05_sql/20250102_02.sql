...
-- Grouping은 조건에 따라 데이터를 그룹으로 묶는 것으로 (WHERE과는 차이가 있음),
-- 데이터를 그룹화 하면 함수로 원하는 계산이 가능

-- 예를 들어 실습 데이터에서 회원수를 구하려면 어떻게?, 전체 회원수가 아니라, 
...
-- 전체 데이터를 통째로 함수 적용하는 경우도 있지만, 그룹별로 수치를 도출하는 일도 있는데, 
...
-- GROUP BY로 계산한 결과를 필터링 하는 HAVING 명령어
-- GROUP BY를 적용해서 '국가별 회원 수를 구하고' 이후에 '그리고 회원 수가 10명 이상인 국가만 도출하라' 
-- 는 등 집계 함수로 계산한 결과 중 조건에 맞는 데이터만 필요한 경우에는 
...
SELECT count(*) 				-- COUNT(컬럼명) 집계 함수에 해당
	FROM users					-- 대상 테이블 전체 행의 개수를 세는 역할
	;

-- USERS에서 존재하는 country 컬럼의 데이터 개수 출력
SELECT count(country)	-- country 컬럼에서 null이 아닌값의 개수, 즉 중복도 포함
	FROM users
	;
...
일반 함수는 SELECT 뿐만 아니라 WHERE에서도 사용 가능.
이상의 함수 예씨들은 전부 다가 아니라 일부이므로 필요시마다 소개하도록 하겠습니다. 

GROUP BY
	-- 어떤 기준으로 묶어서 계산하느냐와 관련이 있는 키워드로, 
...
-- 그 후 집계 함수는특정 컬럼을 기준으로 데이터를 그룹화 한 후에, 그룹별로 집계 함수를 
-- 적용해야 합니다. 
...
-- 모든 국가를 대상으로 회언 수를 확인해 보기 위해서는 국가별로 즉, 
-- 총 17번의 SQL문을 작성해야 합니다. 
-- 국가 수는 적지도 않고, 실제 환경에서는 훨씬 더 많은 데이터를 다루는데, 조건을 일일이 
-- 적용하기에는 비효율적입니다. 
-- 즉, where 절에서는 country=의 값을 매번 바꾸는 방식은 사용하지 않습니다. 
