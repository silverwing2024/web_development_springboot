...
/*
    1. 사전 지식 : API와 REST API
        ...
        식당으로 알아보는 API
            여러분이 손님인데, 식당에 들어갔어요 -> 점원에게 요리를 주문합니다.(주방으로 달려가서 요리를 하는게 아님).
                ...
                REST API를 사용하는 방법
                    규칙 1. URL에는 동사를 쓰지 말고, 자원을 표시한다.
                     ...
                     동사를 사용해서 생기게 되는 추후의 문제점 예시 -> 데이터를 요청하는 URL을 설계할 때
                        A 개발자는 get, B 개발자는 show를 쓰면 get-student, show-date등으로 협의가 이루어지지 않은 설계가
                        될 가능성이 있다.
                      ...
                      규칙 2. 동사는 HTTP 메서드로
                         ...
                         *GET / POST 등은 URL에 입력하는 값이 아니라 내부적으로 처리하는 방식을 미리 정하는 것으로
                         ...
                         도큐먼트인지에 따라 단수, 복수를 나누거나 하는 등의 규칙이 있지만 마찬가지로 추후 설명 예정.
