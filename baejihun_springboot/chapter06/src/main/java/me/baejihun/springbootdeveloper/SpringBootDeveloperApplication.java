    ...
    1. 데이터베이스(database)란?
        ...
        DBMS(DataBase Management System): 기본적으로 데이터베이스는 많은 사람이 공유할 수 있어야 하고,
            동시 접근이 가능해야 하는 등 많은 요구 사항이 존재함.
                이를 만족시키면서도 효율적으로 데이터베ㅣ스를 관리하는 체계가 DBMS.
                대부분 개발자들이 편하게 DB라고 이야기하는 MySQL, Oracle, DBeaver같은 것들은
                DBMS에 해당
            ...
            관계형 DBMS
               ...
               +----------------------------------+
                ...
                H2, MySQL
                    ...
                    H2 - 자바로 작성되어 있는 RDBMS: 스프링부트가 지원하는 인메모리 관계형 데이터베이스
                ...
                필수 용어
                    ...
                    3) 열(Column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                        예를 들어 회원테이블이 있다고 할 때. 열은 각 요소에 대한 속성을 나타내며(Java에서는 field)
                        무결성을 보장함. 이상의 표를 기준으로 할 때는, 이메일은 문자열일 것, 나이는 숫자 유형을 가짐.
                        이메일 옆에 숫자가 들어가거나, 나이 열에 문자열이 들어갈 수 없기 때문에 데이터에 대한 무결성을 보장.
                    4) 기본키(Primary Key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 '중복값을 가질 수 없음'
                        보통 데이터를 수정하거나 삭제하고, 조회할 때 사용되며 다른 테이블과 관계를 맺어 데이터를 ㅏ져올 수도 있음(Join 개념)
            ...
            2. ORM이란?
                Object-Relational Mapping이라고 하며 자바의 객체와 데이터베이스를 연결하는 프로그래밍 기법
                    예를 들어 데이터베이스에 age, name 칼럼에 20, 홍길동이라는 값이 있다고 가정했을 때, 이 것을 자바에서 사용하려면
                    SQL을 이용하여 데이터를 꺼내 사용하지만, ORM이 있다면 데이터베이스의 값응ㄹ 마치 객체처람 사용할 수 있음.
                ...
                장점:
                    ...
                    4) 매핑하는 정보가 명확히가 때문에(DB값과 인스턴스 변수의 값), ERD에 대한 의존도를 낮출 수 있고 유지보수 시 유리

                단점:
                    1) 프로젝트의 복잡성이 커질 수록 사용 난이도가 올라감.
                    2) 복잡하고 무거운 쿼리는 ORM으로 해결이 불가능한 경우가 있음.(JOIN이 여러번 들어가거나 / 서브쿼리가 복잡한 경우)
