...
class MemberRepositoryTest {
    ...
    /*
        이상의 코드는 멤버 전체를 찾고, 그 크기가 리스트의 사이즈가 3인지를 확인하는 테스트

        이하의 코드는 id가 2인 멤버를 찾아보도록 합니다.
        ex)
            SELECT * FROM member WHERE id = 2;
            member라는 테이블의 모든 칼럼을 조회하는데, id가 2인 객체의 값들을 보여달라.
    ...
    */
    ...
    /*
        ...
        일반적인 경우 JPA가 정해준 메서드 이름 규칙을 따르면 쿼리문을 특별히 구현하지 않아도 메서드처럼 사용할 수 있음
        (즉 자바만으로 컨트롤이 가능하다는 의미)
        ...
        전체 조회 -> findAll()
        아이디로 조회 -> findById()
        특정 칼럼으로 조회(name) -> 쿼리 메서드 명명 규칙에 맞게 정의 후에 사용(find / get 차이)
    ...
    @Test   // @Sql애너테이션을 달지 않습니다. 이 메서드에서는 1, 'A'에 해당하는 객체를 저장할거라서
    void saveMember(){
        ...
        //when
        memberRepository.save(member);  //save가 자동완성됐다는 점에 주목해야 합니다.
